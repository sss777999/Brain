...
# ANCHOR: BABI_ANSWER
# API_PRIVATE
def answer_babi_question(question: str, task_num: int) -> str:
    """
    Produce the raw Brain answer for a bAbI question.

    Intent: Keep bAbI routing logic in one place so both standalone and unified
    harnesses evaluate the same retrieval behavior.

    Args:
        question: Question string.
        task_num: Which bAbI task this is.

    Returns:
        Raw lowercased Brain answer.

    Raises:
        AssertionError: If inputs are invalid.
    """
    assert question.strip(), "question must be non-empty because bAbI evaluation requires a concrete query"
    assert 1 <= task_num <= 20, "task_num must stay within the bAbI range because routing logic depends on a valid task id"
    import train

    if task_num in (2, 3, 16):
        raw_answer = train.ask_multi_hop(question).lower().strip()
    else:
        raw_answer = train.ask(question).lower().strip()

    assert isinstance(raw_answer, str), "bAbI answering must return a string because evaluation compares linguistic outputs"
    return raw_answer


# ANCHOR: BABI_TESTER  
def test_question(question: str, expected: str, task_num: int) -> Tuple[bool, str]:
    """
    Test single question against Brain using train.ask() (episodic retrieval + PFC).
    ...
    Args:
        ...
        task_num: Which bAbI task this is (for routing)
        
    Returns:
        Tuple of (is_correct, actual_answer)
    """
    assert question.strip(), "question must be non-empty because bAbI evaluation needs an explicit query"
    assert expected.strip(), "expected must be non-empty because pass/fail depends on a concrete target"
    raw_answer = answer_babi_question(question, task_num)
    from test_brain import check_answer

    is_correct = check_answer(raw_answer, expected, question)
    assert isinstance(is_correct, bool), "bAbI correctness checks must yield a boolean because benchmark accounting is binary"
...
def run_babi_task(
    ...
    # Find task file (use TEST files to prevent data leakage)
    task_files = list(Path(data_dir).glob(f"qa{task_num}_*_test.txt"))
    if not task_files:
        raise FileNotFoundError(f"No test file found for task {task_num} in {data_dir}")
    
    task_file = task_files[0]
    task_name = task_file.stem.replace("_test", "")
    ...
    for i, story in enumerate(stories):
        ...
        for qa in story["qa"]:
            ...
            is_correct, actual = test_question(question, expected, task_num)
            total_questions += 1
