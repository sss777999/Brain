...
SENTENCES = [
    ...
    # =================================================================
    # CONSTRUCTION PATTERN REINFORCEMENT
    # =================================================================
    # BIOLOGY (Construction Grammar, Goldberg 1995; Tomasello 2003):
    # Children learn syntactic constructions through REPEATED EXPOSURE
    # to the same pattern with different content words.
    # Each construction below is repeated 5-15 times to strengthen
    # the syntactic frame (connector) in Broca's area (dorsal stream).
    # This is usage-based learning: frequency builds representation.
    # =================================================================

    # -----------------------------------------------------------------
    # PATTERN: [X] is a [Y] — IS-A categorization (taxonomic)
    # Creates connector "is_a" on X→Y connections
    # -----------------------------------------------------------------
    "a dog is a pet",
    "a cat is a pet",
    "a hamster is a pet",
    ...
    "a lamp is a piece of furniture",
    "a car is a vehicle",
    "a bus is a vehicle",
    "a truck is a vehicle",
    ...
    # -----------------------------------------------------------------
    # PATTERN: [X] is [PROPERTY] — attribution (property assignment)
    # Creates connector "is" on X→PROPERTY connections
    # -----------------------------------------------------------------
    "the sky is blue",
    "the grass is green",
    "the sun is bright",
    ...
    # -----------------------------------------------------------------
    # PATTERN: [X] is in the [Y] — static location (locative)
    # Creates connector "is_in_the" on X→Y connections
    # -----------------------------------------------------------------
    ...
    # -----------------------------------------------------------------
    # PATTERN: [X] lives in the [Y] — habitat (animate location)
    # Creates connector "lives_in_the" on X→Y connections
    # -----------------------------------------------------------------
    ...
    "a lion lives in the savanna",
    "a zebra lives in the savanna",
    "a giraffe lives in the savanna",
    "an eagle lives in the mountains",
    "a frog lives near the pond",
    "a duck lives near the pond",
    "a crab lives near the ocean",

    # -----------------------------------------------------------------
    # PATTERN: [X] [VERB]s on the [Y] — action at location
    # Creates connectors like "on_the" between verb/X and Y
    # -----------------------------------------------------------------
    ...
    # -----------------------------------------------------------------
    # PATTERN: [X] has [Y] — possession / part-whole
    # Creates connector "has" on X→Y connections
    # -----------------------------------------------------------------
    "a dog has four legs",
    "a cat has four legs",
    ...
    "a human has one nose",
    "a human has one mouth",
    "a human has two hands",
    "a human has two feet",
    "a human has ten fingers",
    "a human has ten toes",

    # -----------------------------------------------------------------
    # PATTERN: [X] is made of [Y] — material composition
    # Creates connector "is_made_of" on X→Y connections
    # -----------------------------------------------------------------
    ...
    # -----------------------------------------------------------------
    # PATTERN: the [PROPERTY] of [X] is [Y] — property extraction
    # Creates connectors "of" on PROPERTY→X and "is" on X→Y
    # -----------------------------------------------------------------
    ...
    # -----------------------------------------------------------------
    # PATTERN: [X] eats [Y] — agent-action-patient (transitive)
    # Creates connector between X and Y via verb
    # -----------------------------------------------------------------
    "a cow eats grass",
    "a horse eats hay",
    ...
    "a dog eats meat",
    "a frog eats insects",
    "a bear eats berries and fish",
    "a squirrel eats nuts",
    "a goat eats grass",
    "a chicken eats grain",
    "a pig eats almost anything",
    "a mouse eats cheese",
    "a spider eats insects",

    # -----------------------------------------------------------------
    # PATTERN: [X] can [VERB] — ability / capability
    # Creates connector "can" on X→VERB connections
    # -----------------------------------------------------------------
    "a bird can fly",
    "a fish can swim",
    "a dog can run",
    ...
    # -----------------------------------------------------------------
    # PATTERN: [X] [VERB]s during the [TIME] — temporal
    # Creates connector "during_the" on VERB→TIME connections
    # -----------------------------------------------------------------
    "the sun shines during the day",
    "the moon shines during the night",
    "stars appear during the night",
    "owls hunt during the night",
    "roosters crow during the morning",
    "bats fly during the night",
    "flowers open during the day",
    "we sleep during the night",
    "we work during the day",
    "we eat breakfast during the morning",
    "we eat lunch during the afternoon",
    "we eat dinner during the evening",

    # =================================================================
    # CHILD-DIRECTED SPEECH (CDS) — SITUATED LEARNING
    # =================================================================
    # BIOLOGY (Fernald 1989, Snow 1972, Hart & Risley 1995):
    # Children hear ~30,000 words/day in natural contexts.
    # CDS is simpler, slower, more repetitive than adult speech.
    # Words are learned IN CONTEXT — kitchen, playground, bedtime.
    # The same constructions repeat across many situations.
    # =================================================================

    # -----------------------------------------------------------------
    # CONTEXT: MORNING ROUTINE
    # Child hears these every day — extreme repetition strengthens
    # -----------------------------------------------------------------
    "it is morning",
    "the sun is up",
    "time to wake up",
    ...
    "we use toothpaste to brush our teeth",
    "we comb our hair",
    "we get dressed",
    ...
    "we swallow food",
    "we say please when we want something",
    "we say thank you when we get something",
    ...
    "a sandwich has bread and filling",
    "fruit is healthy",
    "vegetables are healthy",
    ...
    "we mix ingredients in a bowl",

    # -----------------------------------------------------------------
    # CONTEXT: GETTING DRESSED / CLOTHING
    # -----------------------------------------------------------------
    "a shirt goes on your body",
    "pants go on your legs",
    "socks go on your feet",
    "shoes go on your feet",
    "a hat goes on your head",
    "gloves go on your hands",
    "a scarf goes around your neck",
    "a coat keeps you warm",
    "a raincoat keeps you dry",
    "boots are for rain and snow",
    "sandals are for summer",
    "we wear warm clothes in winter",
    "we wear light clothes in summer",
    "we wear a swimsuit to swim",
    "we wear pajamas to sleep",
    "clothes are made of fabric",
    "fabric can be cotton or wool",
    "wool comes from sheep",
    "cotton comes from plants",
    "silk comes from silkworms",

    # -----------------------------------------------------------------
    # CONTEXT: THE HOUSE / ROOMS
    # -----------------------------------------------------------------
    "a house has many rooms",
    "the kitchen is where we cook",
    "the bedroom is where we sleep",
    "the bathroom is where we wash",
    "the living room is where we sit and relax",
    "the dining room is where we eat",
    "the garage is where we park the car",
    ...
    "the door is how we enter",
    "the window lets in light",
    "we turn on the light when it is dark",
    "we turn off the light when we sleep",
    "a bed is in the bedroom",
    "a sofa is in the living room",
    "a stove is in the kitchen",
    "a sink is in the kitchen and the bathroom",
    "a bathtub is in the bathroom",
    "a mirror is in the bathroom",
    "a television is in the living room",
    "a bookshelf has many books",

    # -----------------------------------------------------------------
    # CONTEXT: PLAYGROUND / OUTSIDE
    # -----------------------------------------------------------------
    "we go outside to play",
    "the playground is in the park",
    "a slide is for sliding down",
    "a swing goes back and forth",
    "a seesaw goes up and down",
    "we run on the grass",
    "we jump over puddles",
    "we climb on the monkey bars",
    "we throw the ball",
    "we catch the ball",
    "we kick the ball",
    "a ball is round",
    ...
    "an eagle has very sharp eyes",
    "an owl can turn its head very far",
    ...
    "we have one nose to smell",
    "we have one mouth to eat and talk",
    "we have teeth inside our mouth",
    "we have a tongue to taste",
    "we have two arms",
    "each arm has a hand at the end",
    "each hand has five fingers",
    "we use our fingers to hold things",
    "our thumb helps us grip",
    "we have two legs to walk and run",
    "each leg has a foot at the end",
    "each foot has five toes",
    "our knees bend when we walk",
    "our elbows bend when we lift things",
    "our heart pumps blood through our body",
    "our lungs help us breathe",
    "our stomach digests food",
    "our bones hold our body up",
    "our muscles help us move",
    "our skin covers our whole body",

    # -----------------------------------------------------------------
    # CONTEXT: WEATHER AND SEASONS (detailed)
    # -----------------------------------------------------------------
    "when it is sunny the sky is clear",
    "when it is cloudy the sky is gray",
    "when it rains the ground gets wet",
    "when it snows the ground turns white",
    "when it is windy the trees move",
    "when it is foggy we cannot see far",
    "thunder is the sound of a storm",
    "lightning is a flash of light in the sky",
    "a storm has wind and rain",
    "a blizzard is a storm with snow",
    "hail is balls of ice that fall from the sky",
    "frost forms on cold mornings",
    ...
    "in winter some animals hibernate",
    "bears hibernate during the winter",
    "birds migrate to warmer places in winter",
    "some trees lose their leaves in winter",

    # -----------------------------------------------------------------
    # CONTEXT: TRANSPORT AND TRAVEL
    # -----------------------------------------------------------------
    "a car has four wheels and an engine",
    "a car drives on the road",
    "we wear a seatbelt in the car",
    "a bus is bigger than a car",
    "a bus carries many people",
    "a bus stops at the bus stop",
    "a truck carries heavy things",
    "a train rides on the tracks",
    "a train stops at the station",
    "a subway train goes underground",
    "an airplane flies in the sky",
    "an airplane takes off from the airport",
    "an airplane lands at the airport",
    "a helicopter can hover in the air",
    "a boat floats on the water",
    "a ship is a very big boat",
    "a sailboat uses wind to move",
    "a submarine goes under the water",
    "a rocket goes into space",
    "a bicycle is good for short trips",
    "we walk when the place is close",
    "we take a car when the place is far",
    "a fire truck carries firefighters",
    "an ambulance carries sick people to the hospital",
    "a police car helps keep us safe",

    # -----------------------------------------------------------------
    # CONTEXT: PROFESSIONS — what people do
    # -----------------------------------------------------------------
    ...
    "a firefighter puts out fires and saves people",
    "a police officer catches bad people",
    "a farmer grows food on the farm",
    "a baker bakes bread and cakes",
    "a chef cooks food in a restaurant",
    "a builder builds houses and buildings",
    "a painter paints walls and pictures",
    "a driver drives a bus or a taxi",
    "a pilot flies an airplane",
    "a sailor sails on boats and ships",
    "an astronaut travels to space",
    "a scientist studies how things work",
    "a vet takes care of sick animals",
    "a librarian works in the library",
    "a mailman delivers letters and packages",
    "a musician plays music",
    "a singer sings songs",
    "an artist makes paintings and drawings",
    "a writer writes books and stories",
    "an engineer designs and builds machines",
    "a plumber fixes pipes and water systems",
    "an electrician fixes electrical things",

    # -----------------------------------------------------------------
    # CONTEXT: SCHOOL AND LEARNING
    # -----------------------------------------------------------------
    "we go to school to learn",
    "the teacher stands at the front of the class",
    "students sit at their desks",
    "we learn to read and write at school",
    "we learn math at school",
    "we learn science at school",
    "we learn history at school",
    "a classroom has a board and desks",
    "we write on paper with a pencil",
    "we write on the board with chalk",
    "we use an eraser to fix mistakes",
    "we read books to learn new things",
    "a library has many books",
    "we borrow books from the library",
    "we raise our hand to ask a question",
    "we listen when the teacher talks",
    "homework is work we do at home",
    "a test shows what we have learned",
    "recess is when we go outside to play",
    "lunch is when we eat at school",
    "the school bell rings to start class",

    # -----------------------------------------------------------------
    # CONTEXT: NUMBERS AND COUNTING
    # -----------------------------------------------------------------
    ...
    "yellow is the color of a lemon",
    "green is the color of grass",
    "green is the color of leaves",
    "green is the color of a frog",
    "blue is the color of the sky",
    ...
    "gray is the color of a rainy sky",
    ...
    "a heart is a shape that means love",
    "a diamond has four sides like a tilted square",
    "an oval is like a stretched circle",

    # -----------------------------------------------------------------
    # CONTEXT: FEELINGS AND EMOTIONS
    # -----------------------------------------------------------------
    "when we are happy we smile",
    "when we are sad we cry",
    "when we are angry we feel mad",
    "when we are scared we feel afraid",
    "when we are tired we yawn",
    "when we are hungry we want to eat",
    "when we are thirsty we want to drink",
    "when we are cold we shiver",
    "when we are hot we sweat",
    "when we are sick we feel bad",
    "when we are healthy we feel good",
    "when we are excited we feel very happy",
    "when we are bored we have nothing to do",
    "when we are surprised we say wow",
    "when we are proud we feel good about what we did",
    "when we are shy we do not want to talk",
    "when we are lonely we feel alone",
    "when we are grateful we say thank you",
    "friends make us feel happy",
    "hugs make us feel better",
    "kind words make us feel good",
    "we should be kind to others",
    "we should help others when they need it",
    "we should share with our friends",
    ...
    "we use soap to wash our body",
    "we use shampoo to wash our hair",
    "we dry ourselves with a towel",
    "we put on our pajamas",
    "we brush our teeth before bed",
    "we read a bedtime story",
    "we say goodnight to our family",
    "we turn off the light",
    "we lie down in our bed",
    "we close our eyes",
    "we fall asleep",
    "we dream when we sleep",
    "the moon and stars are in the sky at night",
    ...
    "we go at a green light",
    "we wait at a yellow light",
    "we wear a helmet when riding a bike",
    "we wear a seatbelt in the car",
    "we do not play with fire",
    "we do not touch hot things",
    "we do not talk to strangers",
    "we do not run near the pool",
    "we stay close to our parents in public",
    "we call for help when we are in danger",
    "a doctor helps when we are hurt",
    "a firefighter helps when there is a fire",
    "a police officer helps when we are lost",

    # -----------------------------------------------------------------
    # CONTEXT: MANNERS AND SOCIAL RULES
    # -----------------------------------------------------------------
    "we say please when we ask for something",
    "we say thank you when someone helps us",
    "we say sorry when we make a mistake",
    "we say excuse me when we need to pass",
    "we say hello when we meet someone",
    "we say goodbye when we leave",
    "we take turns and do not push",
    "we listen when others are talking",
    "we do not interrupt when someone is speaking",
    "we raise our hand when we want to speak",
    "we share our things with friends",
    "we are kind to everyone",
    "we do not hit or hurt others",
    "we tell the truth",
    "we help people who need help",
    "we clean up after ourselves",

    # -----------------------------------------------------------------
    # CONTEXT: EARTH AND SPACE (simple)
    # -----------------------------------------------------------------
    "the earth is a planet",
    ...
    "space is very big and very dark",
    "astronauts travel to space in a rocket",
    ...
    # -----------------------------------------------------------------
    # CONTEXT: SIMPLE SCIENCE
    # -----------------------------------------------------------------
    "water can be liquid ice or steam",
    "ice is frozen water",
    "when ice gets warm it melts into water",
    "when water gets very hot it turns into steam",
