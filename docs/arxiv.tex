...
\date{
  \vspace{0.5em}
  \normalsize{DOI: \href{https://doi.org/10.5281/zenodo.18704593}{10.5281/zenodo.18704593}}
}
...
On a curated curriculum of basic world knowledge and question answering, the system achieves 100\% accuracy (705/705 questions across 6 QA test suites + 20 bAbI working memory tasks), including 100\% on all 20 bAbI tasks (481/481), while appropriately abstaining on unknown queries. Comparison with standard IR baselines (TF-IDF, BM25) on identical training data shows Brain outperforms QA baselines by +43--49\%. Working memory baselines (MemNet, NTM) tested on all 20 bAbI tasks average 24.3\% and 19.4\% respectively vs Brain's 100\%. The architecture and evaluation pipeline are released as open-source code at \url{https://github.com/sss777999/Brain}.
\end{abstract}
...
The same principle is applied during location retrieval: when prefrontal cortex requests
``Where is X?'', CA3 biases competition toward episodes that carry explicit spatial evidence
for the tracked subject, while same-subject possession episodes without spatial content lose
their retrieval advantage. During retrospective replay, this spatial facilitation becomes
stronger, helping the system recover the carrier's location at the moment an object was
released rather than perseverating on object-handling episodes.
...
On the bAbI Task 1 benchmark (single supporting fact with distractor sentences), this architecture achieves 100\% accuracy without any task-specific training---the same mechanisms that maintain any working memory representation naturally handle the task structure.

\subsubsection{Extended bAbI: Tasks 1--20 via PFC Situation Model}

Beyond simple working memory (Task~1), the full bAbI benchmark \citep{weston2015towards} tests 20 cognitive abilities including multi-hop reasoning, coreference resolution, counting, negation, temporal and spatial reasoning, deduction, induction, and motivation inference. The current architecture addresses these families through two additional biologically motivated mechanisms, with targeted task-family validation reaching 100\% on the affected suites.

\textbf{Phase 22: Coreference Resolution.} Broca's area maintains a discourse model of active referents \citep{hagoort2005broca}. Pronouns (he/she/they) are bound to antecedents via gamma-band synchronization between Broca's area and temporal cortex \citep{fries2005mechanism}. The resolver tracks the most recently mentioned male, female, and plural entities---consistent with recency-based discourse models \citep{howard2002distributed}. This enables Tasks 11 (basic coreference) and 13 (compound coreference).

\textbf{Phase 23: Core PFC Situation Readouts.} The prefrontal cortex maintains a structured situation model---a mental representation of ``who is where,'' ``who has what,'' and ``which drive is currently active'' \citep{baddeley2000episodic, goldman1995cellular}. In the current implementation, Broca's area parses state-sensitive query forms and \texttt{train.ask()} performs direct working-memory readout before episodic fallback. The maintained state supports:
\begin{itemize}
  \item Entity location tracking (PFC spatial register, \citealt{goldman1995cellular})
  \item Object possession and transfer tracking (visuospatial sketchpad, \citealt{baddeley2000episodic})
  \item Motivation drive tracking with drive-linked destination/object goals
  \item Negation processing (PFC inhibitory control, \citealt{miller2001integrative})
  \item Temporal history with time cell ordering (\citealt{eichenbaum2014time})
  \item Spatial reasoning with transitive inference via cognitive maps (\citealt{okeefe1978hippocampus})
  \item Path finding via place/grid cell navigation (\citealt{moser2008place})
  \item Type-based deduction via semantic memory hierarchy (\citealt{collins1969retrieval})
\end{itemize}

\noindent\textbf{Architectural boundary.} The benchmark harness (\texttt{test\_babi.py}) loads stories and scores answers, but the authoritative situation-model inference now lives in core brain code (primarily \texttt{broca.py} and \texttt{train.py}) as task-selective PFC-style working-memory readouts. Episodic retrieval remains the fallback path when the maintained state is insufficient.

\noindent\textbf{Known limitation.} Some canonical drive-to-goal associations remain explicit priors that would ideally be grounded in learned semantic memory during training. This is a pragmatic simplification analogous to rule-based parsing in Broca's area---the \emph{mechanism} is biologically grounded, while part of the \emph{goal vocabulary} is still explicitly provided.
...
\textbf{Selective Inclusion (Phase~21):} Source memory does not implement a hard gate
that blocks non-preferred sources entirely. Instead, preferred sources are always
included in the candidate pool, while non-preferred sources (e.g., MEDIA) are
included \emph{only} when they contain ALL content query words---indicating a
highly specific match rather than coincidental overlap. This mirrors how the brain
preferentially retrieves from trusted sources but can still recall less-trusted
information when it is strongly associated with the query. Combined with
trust-weighted scoring (\texttt{trust\_multiplier}), this provides a biologically
plausible hierarchy where trusted sources dominate for common knowledge, while
domain-specific information from less-trusted sources remains accessible when
trusted sources lack relevant knowledge.
...
The \texttt{training\_modes.py} module defines 22 training modes based on brain memory systems:

\textbf{TrainingMode}: Enumeration of declarative and procedural training modes including FACT, DEFINITION, HIERARCHY, PROPERTY, SELF\_FACT, RELATION, SEQUENCE, EPISODE, SELF\_EPISODE, PROCEDURE, ROUTINE, CAUSE\_EFFECT, and others.

Each mode has corresponding data classes (FactData, DefinitionData, HierarchyData, SelfFactData, SelfEpisodeData, etc.) that structure input for appropriate encoding. The self-related modes separate stable self-schema from autobiographical episodes while keeping both compatible with the same hippocampal encoding pathway.
Rather than directly forcing replay state changes, self-related modes pass newly encoded traces through a hippocampal self-integration routine that assigns salience, ownership confidence, replay priority, and autobiographical continuity links before replay unfolds during quiet wake or sleep.
...
\textbf{Episode}: Each episode stores: (1) \texttt{input\_neurons}---original words as frozen set for search; (2) \texttt{input\_words}---word order tuple preserving hippocampal time cell sequence; (3) \texttt{pattern\_neurons}---sparse DG representation ($\sim$2\%); (4) \texttt{context\_neurons}---what was active at encoding; (5) \texttt{timestamp}; (6) \texttt{source}---where/how learned; (7) \texttt{memory\_domain}---GENERAL vs SELF\_SEMANTIC vs SELF\_EPISODIC; (8) \texttt{identity\_tag}---stable self anchor for autobiographical traces; (9) \texttt{state}---current consolidation stage; (10) \texttt{replay\_count}---times replayed during sleep.

This separation allows the system to distinguish ``knowledge about the world'' from ``knowledge about itself'' without changing the basic hippocampal indexing mechanism. First-person lexical variants are canonicalized to a stable \texttt{self\_entity} token before storage and retrieval, preventing fragmentation of autobiographical traces across surface forms such as ``I,'' ``me,'' and ``my.''
Self-related episodes additionally maintain explicit self-model metadata: \texttt{memory\_owner}, \texttt{ownership\_confidence}, \texttt{salience\_level}, \texttt{replay\_priority}, \texttt{autobiographical\_links}, and \texttt{metacognitive\_uncertainty}. These variables are serialized with the model so replay priority and autobiographical continuity survive save/load cycles.
...
\item \textbf{Top-down connector modulation}: Two modes depending on connector type.
      \emph{Biased competition} (string connector, e.g., \texttt{is\_a}): $\times 5.0$
      enhancement for matching connections, $\times 0.2$ suppression otherwise
      \citep{desimone1995neural}. \emph{Soft attentional facilitation} (frozenset connector,
      e.g., temporal \texttt{\{before, after, during, ...\}}): $\times 2.0$ boost for
      matching connections only, no suppression \citep{miller2001integrative}.
...
\item \textbf{Subject bonus}: Episodes containing the question's grammatical subject
      receive additional weight, extracted by Broca's area syntactic processing.
\item \textbf{Self-memory facilitation}: When the query is self-referential, episodes tagged
      as \texttt{SELF\_SEMANTIC} or \texttt{SELF\_EPISODIC} receive additional support, biasing
      recall toward autobiographical and self-schema traces rather than unrelated world knowledge.
      The bonus is modulated by ownership confidence, autobiographical continuity, and
      metacognitive uncertainty, and the source filter is relaxed for self-tagged traces so
      autobiographical experience is not discarded by generic source priors.
\item \textbf{Semantic role bonus}: PFC specifies expected roles (category, location, time,
      agent); episodes with matching roles receive priority \citep{binder2009semantic}.
\item \textbf{Temporal concept inference (Phase~19)}: For ``when'' questions, episodes
      containing temporal nouns (morning, autumn, etc.) not already in the query receive
      a scoring bonus via hippocampal time cell activation \citep{eichenbaum2014time}.
\item \textbf{Source memory filtering (Phase~21)}: Preferred sources always scored;
      non-preferred sources scored only if ALL content query words present
      \citep{johnson1993source}. Source preference bonus for trusted sources.
\item \textbf{Episode deduplication (Phase~20)}: Top-$K$ candidates deduplicated by
      content before CA1 blending, preventing consolidated copies from monopolizing
      the population code \citep{born2012system}.
...
For self-queries, CA1 applies an additional domain-specific readout constraint: stable identity questions are read out from \texttt{SELF\_SEMANTIC} attractors, whereas autobiographical questions are narrowed to context-matching \texttt{SELF\_EPISODIC} traces. This reduces contamination between self-schema and event memories during answer generation.
During focused question answering, CA1 also performs a task-set sharpening step before motor output. The winning top-$K$ attractor population is filtered so that non-verb query context remains covered during blending, preventing semantically related but task-irrelevant narrative traces from leaking into the answer. When the retrieved episode contains a reliable bound role or attribute, CA1 emits a compact answer span---for example a person identity, an age phrase, or a color token---instead of replaying the entire supporting sentence. If no reliable person-bound answer can be isolated, the system falls back to ``I do not know,'' preserving anti-hallucination behavior through the same biased-competition and top-down control principles that guide retrieval \citep{miller2001integrative,desimone1995neural}.
...
recency, salience, replay priority, and uncertainty, consistent with findings that not all memories replay each night
\citep{wilson1994reactivation}.
Self-related traces can also trigger a quiet-wake replay cycle immediately after encoding, providing a biologically plausible bridge between online self-reference processing and later sleep-dependent consolidation.
...
An additional self-query classifier separates stable identity/property questions (e.g., ``Who am I?'') from autobiographical event, time, and location questions. This keeps self-schema retrieval aligned with semantic roles while routing autobiographical questions toward episodic traces even when first-person surface forms vary.

Paraphrase robustness was dramatically improved by implementing Broca's area Phase~3
reanalysis \citep{friederici2011brain}, which normalizes non-canonical question forms
to canonical WH-questions before semantic processing. This includes inversion recovery
(Trace Deletion Hypothesis, \citealt{grodzinsky2000neurology}), classifier stripping
\citep{croft2001radical} (e.g., ``The sun is a type of what?'' $\rightarrow$ ``What is the sun?''), passive decomposition, possessive decomposition, and temporal
embedding. Current paraphrase accuracy is 100\% (50/50), up from 50\% (25/50).
...
|        - Top-down connector: string x5/x0.2, frozenset x2 only          |
|        - Temporal concept bonus: 'when' questions (Phase 19)             |
|        - Subject bonus + semantic role bonus (PFC top-down)              |
|        - Recency: working memory episodes get timestamp bonus            |
|        - Source trust: trust_multiplier + source_bonus                   |
|        - Unconnected context filter: hard skip (lateral inhibition)      |
|        - Episode dedup in top-K by input_words (Phase 20)                |
...
The system implements a \texttt{SequenceGenerator} class that preserves the temporal order stored in hippocampal time cells. When an episode is retrieved, its \texttt{input\_words} tuple maintains the original sequence of \emph{content words} as encoded. Function words (articles, prepositions, copulas) are not stored in episodic memory but are reconstructed during speech production by restoring \emph{connectors} from the learned connection graph. For example, the connection \texttt{capital --[of]--> france} stores the function word ``of'' as a connector; during answer generation, this connector is inserted between the content words, yielding ``capital of france'' rather than ``capital france''. This mirrors the biological Formulator stage of Levelt's speech production model \citep{levelt1989speaking}, where Broca's area reconstructs grammatical structure from lexical frames stored in the dorsal stream \citep{hagoort2005broca,hickok2007cortical}.
...
Statistic & Value \\
\midrule
Neurons (word forms) & 48{,}312 \\
Connections & 1{,}476{,}117 \\
Myelinated connections & 23{,}784 (1.6\%) \\
Used connections & 76{,}342 (5.2\%) \\
New connections & 1{,}375{,}991 \\
Episodic traces & 76{,}679 \\
\quad --- NEW & 35{,}065 \\
\quad --- REPLAYED & 2{,}189 \\
\quad --- CONSOLIDATED & 38{,}074 \\
\quad --- DECAYING & 1{,}351 \\
...
Curriculum QA accuracy & 100\% (50/50) \\
Strict (I do not know) & 100\% (3/3) \\
Preschool QA accuracy & 100\% (48/48) \\
Grade1 QA accuracy & 100\% (64/64) \\
FineWeb QA accuracy & 100\% (9/9) \\
Paraphrase robustness & 100\% (50/50) \\
bAbI Tasks 1--20 (working memory) & 100\% (481/481) \\
Total QA accuracy & 100\% (705/705) \\
{[}INFER-NO-LEARN{]} test & PASS (0 LTM changes) \\
\midrule
\multicolumn{2}{l}{\textbf{Comparison with IR baselines (same training data):}} \\
CURRICULUM: Brain vs TF-IDF/BM25 & +36\%/+30\% (100\% vs 64\%/70\%) \\
PRESCHOOL: Brain vs TF-IDF/BM25 & +19\%/+13\% (100\% vs 81\%/88\%) \\
STRICT: Brain vs TF-IDF/BM25 & +67\%/+67\% (100\% vs 33\%/33\%) \\
GRADE1: Brain vs TF-IDF/BM25 & +34\%/+34\% (100\% vs 66\%/66\%) \\
FINEWEB: Brain vs TF-IDF/BM25 & +89\%/+89\% (100\% vs 11\%/11\%) \\
bAbI 1--20: Brain vs MemNet/NTM* & 100\% vs 24.3\%/19.4\% (avg all 20 tasks) \\
PARAPHRASE: Brain vs TF-IDF/BM25 & +52\% (100\% vs 48\%/48\%) \\
Average advantage & +50\% vs TF-IDF, +47\% vs BM25 \\
\multicolumn{2}{l}{\footnotesize *bAbI: TF-IDF/BM25=0\% (no WM). MemNet/NTM tested on all 20 tasks.} \\
\bottomrule
\end{tabular}
\caption{Model statistics (March 2026). Training pipeline includes source memory stages. PlasticityMode ensures inference does not modify long-term memory. 100\% accuracy on 705 total tests (224 QA + 481 bAbI) is maintained by temporal concept inference (Phase~19), episode deduplication (Phase~20), source memory selective inclusion (Phase~21), and focused CA1 readout sharpening for compact role-bound answers with preserved anti-hallucination. 100\% on all 20 bAbI tasks is achieved through coreference resolution (Phase~22, Broca's area) and PFC situation model (Phase~23). MemNet/NTM baselines tested on all 20 bAbI tasks.}
\end{table}
...
\begin{verbatim}
=== EXCELLENT (10/10) ===
[PASS] Q: What is the opposite of hot? [Brain: 0.001s]
       Brain raw: cold     Expected: ['cold']

[PASS] Q: What does a caterpillar become? [Brain: 0.336s]
       Brain raw: butterfly     Expected: ['butterfly']

[PASS] Q: Who wrote Hamlet? [Brain: 0.573s]
       Brain raw: I do not know     Expected: ['not know'] (correct rejection)

=== PREVIOUSLY FAILING, NOW FIXED ===
[PASS] Q: What disappears from leaves? [Brain: 0.8s]
       Brain raw: green chlorophyll     Expected: ['chlorophyll', 'green']
       (Fixed via Phase 21: source memory selective inclusion)

[PASS] Q: When should you brush your teeth? [Brain: 0.5s]
       Brain raw: day morning and at night     Expected: ['morning', 'night']
       (Fixed via Phase 19: temporal concept inference)

[PASS] Q: What is sedimentary rock made of? [Brain: 0.4s]
       Brain raw: layers sand mud     Expected: ['layers', 'sand']
       (Fixed via Phase 20: episode deduplication)
\end{verbatim}
}

\noindent Pass/fail is determined by keyword matching on ``Brain raw'' output. Full logs with all 705 tests are in the repository.
...
python test_brain.py      # Run all 224 QA + 481 bAbI tests (705 total)
\end{verbatim}
...
\item \textbf{Working memory (PFC)} --- Prefrontal cortex module with temporary connections and episodes.
      ...
      On bAbI Task 1 (single supporting fact), the system achieves 100\% accuracy through working memory with temporal retrieval refinement---distinguishing ``Where is X?'' (recency bias) from ``Where was X?'' (reverse recency).
      Extended to all 20 bAbI tasks (481/481, 100\%) via coreference resolution in Broca's area (Phase~22, \citealt{hagoort2005broca}) and PFC situation model (Phase~23, \citealt{baddeley2000episodic}).
...
\item \textbf{Global Neuromodulator System} --- Dynamic chemical state (\texttt{NeuromodulatorSystem}) representing Dopamine (reward/learning rate), Norepinephrine (attention focus via WTA threshold), Acetylcholine (encode/retrieve mode switching), and Serotonin (PFC gating impulse control) \citep{hasselmo2006role}.
\item \textbf{Synaptic Homeostasis and LTD} --- Biologically grounded forgetting mechanism \citep{tononi2006sleep}. Global downscaling occurs during NREM sleep. Episodes undergo Long-Term Depression (LTD) if not accessed, with purely episodic traces decaying faster than semantic ones. Physical pruning occurs below threshold, bounding memory capacity.
\item \textbf{Narrative Source Filtering} --- Suppresses NARRATIVE associations (stories, fables) during factual retrieval via PFC top-down modulation to prevent memory contamination \citep{tulving1972episodic}.
\item \textbf{Sparse Distributed Representations} --- Words encoded as sparse binary vectors (2048 bits, $\sim$40 active = 2\% sparsity) following HTM theory \citep{hawkins2004intelligence}. SDR overlap captures semantic similarity: similar concepts share active bits. Learned overlap (Hebbian) increases bit sharing between co-occurring words. Connection connectors are encoded as SDRs (bit-based links), enabling pattern matching via natural bit overlap ($\ge 0.75$) without hardcoded string prefix rules. CA3 scoring uses SDR overlap as parallel bonus component, enabling natural generalization.
...
\item \textbf{Temporal Concept Inference (Phase~19)} --- When the PFC sends a \texttt{time} role
      expectation for ``when'' questions, CA3 scoring checks whether episodes contain temporal
      concept nouns (morning, night, autumn, winter, etc.) from a comprehensive set of 89+ words
      covering time-of-day, seasons, months, days, life stages, and temporal adverbs. Only
      \emph{non-query} temporal words receive a bonus---words already in the query are suppressed
      via lateral inhibition (they are already activated and provide no new information).
      This is combined with \textbf{Soft Attentional Facilitation} \citep{miller2001integrative}:
      connections with temporal connectors (\texttt{before}, \texttt{after}, \texttt{during},
      \texttt{while}, \texttt{until}, \texttt{since}) receive a $2\times$ boost during scoring.
      Biology: hippocampal time cells \citep{eichenbaum2014time} encode temporal context;
      PFC top-down modulation biases retrieval toward temporally-tagged episodes.
\item \textbf{Episode Deduplication in Top-K (Phase~20)} --- Consolidated episodes may have
      multiple copies in the hippocampal store (each replay creates a new trace). Without
      deduplication, N copies of the same episode can fill all top-K slots, preventing
      diverse secondary contributions from competing attractors. The system now deduplicates
      \texttt{scored\_candidates} by \texttt{episode.input\_words} before selecting the top-5,
      ensuring each unique content pattern appears at most once. This enables CA1 blending
      to incorporate diverse information from multiple relevant episodes.
      Biology: \citet{born2012system} show that consolidation merges overlapping memory traces
      into unified representations rather than maintaining redundant copies.
\item \textbf{Source Memory Selective Inclusion (Phase~21)} --- Source memory implements a
      biologically plausible retrieval hierarchy. Preferred sources (LEARNING, EXPERIENCE)
      are always included in the candidate pool. Non-preferred sources (MEDIA) are included
      \emph{only} when they contain ALL content query words---a highly specific match that
      indicates genuine relevance rather than coincidental word overlap. This prevents thousands
      of loosely-related MEDIA episodes from overwhelming trusted sources via sheer volume,
      while preserving access to domain-specific MEDIA knowledge when trusted sources lack
      relevant information. Combined with unconnected context filtering (lateral inhibition
      silences episodes where key query words have no structural connections to episode contents),
      this achieves both precision and anti-hallucination.
      Example: ``What disappears from leaves?'' --- LEARNING has no chlorophyll episodes, but
      the MEDIA episode \{green, chlorophyll, disappears, leaves\} contains all query words
      and is selectively included. Meanwhile, ``Who is the president of Mars?'' correctly
      returns ``I do not know'' because no episode (trusted or otherwise) contains both
      \texttt{president} and \texttt{mars}.
\end{itemize}
...
\textbf{Current status: 705/705 tests passing (100\% accuracy) --- 224 QA + 481 bAbI.}

\textbf{Beyond retrieval: a predictive cognitive cycle (thought loop, layer 1 of 3).}
The results reported above are produced by the retrieval-and-scoring pathway. A
separate, opt-in mechanism targets \emph{thought formation} directly: closing a
recurrent loop in which the network's own settled state drives the next retrieval,
rather than a single scored lookup. We implement this as a \emph{predictive
cognitive cycle} (\texttt{cognition.py}, class \texttt{CognitiveCycle}, reached via
\texttt{ask(question, mode="emergent")}; the scripted path remains the default
\texttt{ask(mode="legacy")}). One ``thought tick'' is predictive coding over
discrete states \citep{rao1999predictive,preston2013interplay,wang2001synaptic}:
the prefrontal buffer projects a top-down \emph{prediction} $P_t$ (the set of
neurons reachable from the current cue through strong existing USED/MYELINATED
edges); CA3 settles bottom-up into $S_t$; the \emph{prediction error} is the
structural set difference (surprise $= S_t \setminus P_t$, miss $= P_t \setminus
S_t$); the surprising neurons become the next cue while the prefrontal buffer runs
its own recurrent excitation and decay; and the loop halts when the attractor
reaches a fixed point, the expected semantic role is bound, the state collapses, or
a small tick budget is spent. The number of reasoning steps is therefore
\emph{emergent} rather than scripted, and the mechanism introduces no real-valued
weights, gradients, distance metrics, global search, or language-model calls: the
prediction is a set, the error is a set difference, and ``minimization'' is
attractor settling.

On the trained model this yields the first genuine multi-step composition in the
system. Given ``zorp is in blen'' and ``blen is in quix'' learned as \emph{separate}
facts (never co-stated), the emergent path answers ``where is zorp'' with ``blen is
in quix''---traversing zorp $\rightarrow$ blen $\rightarrow$ quix---whereas the
default path resolves only a single hop. This is an early prototype and layer~1 of a
three-layer program (\textit{all like a brain}): layer~2, inference during sleep
(implemented), in which recency-weighted sharp-wave-ripple replay composes a new
transitive edge A--C from consolidated A--B and B--C across cycles
\citep{kumaran2012inference}---verified to mint the direct shortcut offline,
though surfacing it as a crisp episodic answer awaits a semantic readout path; and
layer~3, a basal-ganglia controller trained by reward-prediction error that decides
when to answer reflexively versus deliberate (implemented): a gate whose per-context
action values are discrete Go/NoGo counts, selected by argmax and updated by a true
reward-prediction error $\delta = \text{realized} - \text{predicted}$ (a dopamine dip
on an expected-good action that fails, a burst on the recovery of a failing one)
\citep{schultz1998predictive}, so the system learns from experience
which queries warrant deliberation; its learning is exercised through an explicit
practice pass rather than yet being auto-wired into live inference. The cycle is not yet the
default answer path, and broader evaluation---including fixing a pre-existing
episode-storage limitation that currently blocks part of the composition
probe---is ongoing.

\textbf{Previously failing tests now resolved:}
\begin{itemize}
\item \textbf{Word Sense Disambiguation} --- ``What is ice?'' and ``What happens when you fall?'' were resolved
      through improved CA3 scoring with subject bonus and cause-effect pattern extraction in Broca's area.
\item \textbf{Temporal reasoning} --- ``When should you brush your teeth?'' resolved via Phase~19
      temporal concept inference, which provides scoring bonus for episodes containing temporal
      nouns (morning, night) combined with soft attentional facilitation for temporal connectors.
\item \textbf{FineWeb retrieval} --- ``What disappears from leaves?'' resolved via Phase~21 source memory
      selective inclusion (MEDIA episodes with all query words are included). ``What is sedimentary
      rock made of?'' resolved via Phase~20 episode deduplication (prevents consolidated copies
      from filling top-K, enabling diverse CA1 blending).
\end{itemize}

\textbf{Why these are structural fixes, not test-specific tuning:}

Each of the three mechanisms that brought accuracy from 93.7\% to 100\% solves a
\emph{class} of problems rather than a specific test case. None contains hardcoded
words, question-specific thresholds, or answer lookups.
...
\toprule
\textbf{Mechanism} & \textbf{Biological basis} & \textbf{Generality} \\
\midrule
Phase~19: Temporal Concept Inference &
  Hippocampal time cells encode temporal context \citep{eichenbaum2014time}.
  PFC top-down modulation biases retrieval toward temporally-tagged episodes
  \citep{miller2001integrative}. &
  Applies to ANY ``when'' question. The 89-word temporal concept set covers
  time-of-day, seasons, months, days, life stages, and temporal adverbs.
  No question-specific logic. \\
\midrule
Phase~20: Episode Deduplication &
  Consolidation merges overlapping memory traces into unified representations,
  not redundant copies \citep{born2012system}. &
  Applies to ALL consolidated episodes. Deduplication by \texttt{input\_words}
  is a generic set operation---any episode with $N$ consolidated copies is
  reduced to 1. \\
\midrule
Phase~21: Source Memory Selective Inclusion &
  Source memory provides retrieval advantage, not a gate
  \citep{johnson1993source}. Lateral inhibition silences weakly-matching
  attractors \citep{desimone1995neural}. &
  Applies to ALL questions with preferred sources. Non-preferred episodes
  pass a generic \texttt{issubset()} check. Anti-hallucination preserved:
  ``Who is the president of Mars?'' correctly returns ``I do not know''
  (no episode contains both words). \\
\bottomrule
\end{tabular}
\caption{Verification that Phase~19--21 mechanisms are biologically grounded
structural fixes, not test-specific tuning. Each mechanism operates on generic
conditions (temporal nouns, duplicate content, query word subset) without
reference to specific test questions.}
\end{table}

To further validate generality, we tested on questions \emph{not present} in any
test suite:
...
Q: What is a cat?                → "animal and a pet that catches..."
\end{verbatim}
}

\noindent These free-form questions were never seen during development. The model
answers correctly from learned knowledge or appropriately abstains, confirming that
the mechanisms generalize beyond the fixed test suite.

\textbf{Remaining architectural limitations:}
\begin{itemize}
...
\item \textbf{Baseline comparison} --- Comprehensive comparison with standard IR baselines (TF-IDF, BM25)
      on identical training data shows Brain significantly outperforms across all QA test suites:
      average +49\% vs TF-IDF, +43\% vs BM25. bAbI Tasks 1--20 achieve 100\% vs 0\% for
      TF-IDF/BM25 (no working memory). MemNet/NTM tested on all 20 bAbI tasks average
      24.3\% and 19.4\% respectively vs Brain's 100\%.
      ...
      Working Memory Baselines (MemNet, NTM): tested on all 20 bAbI tasks.
...
After implementing Broca's area Phase~3 reanalysis, combined with temporal concept inference
(Phase~19) and improved CA3 scoring, paraphrase accuracy reached 100\% (50/50), up from
50\% (25/50) before these improvements.
...
\begin{itemize}
  ...
  \item \textbf{Working Memory Baselines} (MemNet, NTM): tested on all 20 bAbI tasks
\end{itemize}
...
and biological grounding. MemNet averages 24.3\% and NTM 19.4\% across all 20 bAbI tasks 
vs Brain's 100\%.
