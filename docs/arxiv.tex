...
\item \textbf{Baseline comparison} --- Comprehensive comparison with standard IR baselines (TF-IDF, BM25)
      ...
      \textbf{Future baseline expansion (in progress):}
      \begin{itemize}
        \item RAG baseline (Retriever + GPT) --- different class, but fair comparison
        \item Memory Networks (Weston et al. 2015) --- direct comparison on bAbI
        \item Neural Turing Machines (Graves et al. 2014) --- working memory comparison
        \item Transformer-based QA (BERT, RoBERTa) --- fine-tuned on same data
      \end{itemize}
...
\textbf{Evaluation beyond fixed templates.} To quantify generalization, we will introduce a
...
We will also add systematic baseline comparisons with retrieval-augmented generation (RAG),
Memory Networks, and other memory-augmented systems on identical datasets.
